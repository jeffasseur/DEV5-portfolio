...
export default class Bingo {
  ...
  renderCards() {
    ...
    for(teller; teller < this.cards.length; teller++) {
        let card = new Card(this.cards[teller]);
  ...
  static save() {
    ...
    if (cards.length === 0 || cards.length >= 5) {
      ...
      for (let i = 0; i < cards.length; i++) {
        cardsWon.push(cards[i].getAttribute('data-number'));
    ...
    let jsonCardsWon = JSON.stringify({cardsWon});
    localStorage.setItem('bingo', jsonCardsWon);
  ...
  static load() {
    ...
    if (localStorage.getItem("bingo")) {
      ...
      // .bingo__card--done
      let jsonBingo = localStorage.getItem("bingo");
      let bingo = JSON.parse(jsonBingo);
      console.log(bingo);

      for (let i = 0; i < bingo.cardsWon.length; i++) {
        let cardDataNumber = bingo.cardsWon[i];
        console.log(cardDataNumber); // geeft correct getal weer
        let cardDone = document.querySelector(`[data-number='${cardDataNumber}']`);
        cardDone.classList.add('bingo__card--done');
      }
    }
  }
}
